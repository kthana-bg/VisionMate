...
def _load_models_cached():
    """
    Load all six .h5 models once per server session.
    Returns (eye_models_dict, posture_models_dict).
    """
    from utils.model_loader import load_all_eye_models, load_all_posture_models
    return load_all_eye_models(), load_all_posture_models()
...
def init_session_state():
    defaults = {
        ...
        # Model dicts stored here so monitoring_tab can reach them
        "_eye_models":              None,
        "_posture_models":          None,
    }
...
def main():
    ...
    # Load models once and store in session state for monitoring_tab
    eye_models, posture_models = _load_models_cached()
    st.session_state["_eye_models"]     = eye_models
    st.session_state["_posture_models"] = posture_models
    ...
    # Keep processor models in sync (used by local cv2 fallback)
